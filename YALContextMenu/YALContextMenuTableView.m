...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.tableFooterView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 500, 500)]; //Zero rect footer to clear empty rows UITableView draws
...
- (void)showInView:(UIView *)superview withEdgeInsets:(UIEdgeInsets)edgeInsets animated:(BOOL)animated {
    ...
    self.grayView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, superview.frame.size.width, superview.frame.size.height)];
    self.grayView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.7f];
